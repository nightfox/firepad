...
firepad.FirebaseAdapter = (function (global) {
  ...
  FirebaseAdapter.prototype.sendOperation = function (operation, callback) {
    ...
    function doTransaction(revisionId, revisionData) {
      ...
      function sendToMongo(op, uid, timestamp, that) {
          $.ajax({
              type: 'POST',
              url: 'http://localhost:4567/tracker',
              data: {op: op, uid: uid, timestamp: timestamp, path: that.ref_.path.n, revision_id: that.revision_}
          })
      }
    ...
    doTransaction(revisionId, { a: self.userId_, o: operation.toJSON(), t: Firebase.ServerValue.TIMESTAMP });
      sendToMongo(operation.toJSON(), self.userId_, Firebase.ServerValue.TIMESTAMP, this)
  };
